...
for t = 1:T
    ...
    Znorm = sqrt(ZL(:)'*ZLpinv(:) + ZR(:)'*ZRpinv(:));
    lmda = (loss - loss_star)/Znorm;
    lmdas_Polyak(t) = lmda;
    L = L - lmda/Znorm*ZRpinv;
    R = R - lmda/Znorm*ZLpinv;
end
...
fig_name = sprintf('MS_l1_final_n=%d_r=%d_m=%d_ps=%g', n, r, m, ps);
...
legend(lgd, 'Location', 'best', 'Interpreter', 'latex', 'FontSize', 24);
fig_name = sprintf('MS_l1_lambda_n=%d_r=%d_m=%d_ps=%g_q=%g', n, r, m, ps, q);
...
ylabel('Relative error');
legend(lgd, 'Location', 'best', 'Interpreter', 'latex', 'FontSize', 24);
fig_name = sprintf('MS_l1_q_n=%d_r=%d_m=%d_ps=%g_lambda=%g', n, r, m, ps, lmda);
%% Figure (d): stepsizes
figure('Position', [0,0,800,600], 'DefaultAxesFontSize', 20);
...
fig_name = sprintf('MS_l1_stepsizes_n=%d_r=%d_m=%d_ps=%g', n, r, m, ps);
